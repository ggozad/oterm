...
chat_sqlite = """
-- name: save_chat
INSERT OR REPLACE INTO chat(id, name, model, context, template, system) 
VALUES(:id, :name, :model, :context, :template, :system) RETURNING id;
-- name: save_context
UPDATE chat SET context = :context WHERE id = :id;
-- name: rename_chat
UPDATE chat SET name = :name WHERE id = :id;
-- name: get_chats
SELECT id, name, model, context, template, system FROM chat;
-- name: get_chat
SELECT id, name, model, context, template, system FROM chat WHERE id = :id;
-- name: delete_chat
