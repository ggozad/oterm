...
async def sampling_handler(
    ...
    if params.modelPreferences and params.modelPreferences.hints:
        ...
        model_from_hints = search_model(model_hints)
        if model_from_hints:
    ...
    for msg in messages:
        if type(msg.content) is TextContent:
            prompt_parts.append(f"{msg.role}: {msg.content.text}")
    prompt = "\n".join(prompt_parts)
...
def search_model(hints: list[ModelHint]) -> ListResponse.Model | None:
    log.info("Searching for model based on hints", [h.name for h in hints])
    available_models = ollama.list_models().models
